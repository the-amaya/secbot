...
class ServerStuff(commands.Cog):
    ...
    async def on_ready(self):
        print('Cog serverstuff is ready.')
    ...
    @commands.command(name='servers', help=f'print a physical layout of drives for each server with temp and '
                                           ...
                                           f'{list(drivewatch.serverlist.keys())}', hidden=True)
    ...
    @commands.command(name='racsec', help='get a list of drives with reallocated sectors on a specified server', hidden=True)
    ...
    @commands.command(name='badarray', help='get a list of non-OK arrays from all the servers', hidden=True)
    async def badarray(self, ctx):
        async with ctx.typing():
            await ctx.send(await drivewatch.all_status())

    @commands.command(name='baddrive', help='get a list of non-OK drives from one of the servers', hidden=True)
...
async def setup(bot):
    await bot.add_cog(ServerStuff(bot))
